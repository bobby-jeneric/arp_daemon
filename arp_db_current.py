...
class DBCurrent:
	...
	def create_db():
		if not DBLayer.connection:
			ArpDump.printout("DBCurrent: create_db: No connection established!")
			return

		c = DBLayer.connection.cursor()
		c.execute('''create table if not exists vms_current(ip text, mac text, name text)''')
	...
	def store_collection(collection):
		...
		c.executemany("insert into vms_current values(?,?,?)", items )
		DBLayer.connection.commit()
